...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_REQUEST["name"])) {
        $nameErr = "<p style='color:red'> * Nama Diperukan</p>";
        $name = "";
    } else {
        $name = $_REQUEST["name"];
    }
    ...
    if (!empty($name) && !empty($email)) {
        // Koneksi database
        require_once "../connection.php";
        $sql_select_query = "SELECT email FROM employee WHERE email = '$email' AND email != '$session_email'";
        $r = mysqli_query($conn, $sql_select_query);

        if (mysqli_num_rows($r) > 0) {
            $emailErr = "<p style='color:red'> * Email Sudah Terdaftar</p>";
        } else {
            $sql = "UPDATE employee SET name = '$name', email = '$email', dob = '$dob', gender = '$gender' WHERE email = '$session_email'";
            $result = mysqli_query($conn, $sql);
            if ($result) {
